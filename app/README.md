...
主界面右侧工具栏点击 🗺️ **地图管理** 进入。

| 操作 | 说明 |
| --- | --- |
| 切换地图 | 点击 ✓ 图标，后端将对应地图发布到 `/map` 话题并刷新前端瓦片 |
| 编辑地图 | 点击 ✏️ 图标进入障碍/拓扑编辑页 |
| 删除地图 | 点击 🗑️ 图标，确认后删除后端 `~/.maps/<map_name>/` 目录 |
| 建图可视化 | 点击 AppBar「进入建图模式」，实时查看 `/scan`（红色激光点）与 `/map` 叠加效果 |

> 建图由 ROS 后台（如 `slam_toolbox`）负责，软件仅显示，无建图控制指令。  
> 超过 5 秒未收到 `/scan` 数据时，页面会自动弹出 Toast 提示。

### 4.4 多点巡航导航

主界面右侧工具栏点击 🧭 **多点巡航导航** 进入。

| 操作 | 说明 |
| --- | --- |
| 添加导航点 | 在地图上**长按**即可添加（橙色编号圆形标记） |
| 查看/排序/删除 | 点击右上角列表图标打开抽屉面板，支持拖拽排序与单点删除 |
| 保存导航点 | 点击底部「保存」，持久化写入后端对应地图的 `waypoints.json` |
| 开始巡航 | 按编号顺序依次发送导航目标；到达（`succeeded`）后自动发送下一个；60 秒超时自动停止 |
| 取消 | 点击「取消」，发送 `cancel_nav`，机器人原地停止 |
