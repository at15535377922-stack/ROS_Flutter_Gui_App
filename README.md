...
| 位姿 | 后端在地图坐标系下封装位姿并推送 |
| 重定位与导航 | 通过后端 HTTP 发布初始位姿（`/initialpose`）与导航目标；拓扑与地图编辑走 HTTP；主页面与多点巡航页面均支持重定位 |
...
| **多点巡航导航** | 长按地图添加导航点，拖拽排序；顺序执行导航，到达后自动跳转下一点；支持取消（原地停车）；**支持重定位**（拖动机器人图标对齐实际位置后发布 `/initialpose`）；导航点持久化保存；见下文 **§3.4** |
...
| **重定位** | AppBar 点击重定位图标进入重定位模式；拖动机器人图标到实际位置并旋转调整朝向；确认后向 `/initialpose` 发布初始位姿供 amcl 收敛定位 |

> **重定位生效条件**：需在机器人侧运行 `amcl`（订阅 `/initialpose`）与激光雷达驱动（发布 `/scan`），否则发布后小车位置不会更新。
...
- **修复**：兼容 `flutter_map 8.3.0`，移除已废弃的 `TileLayer.wrapAround` 参数，改用 `MapOptions(crs: CrsSimple())` 防止本地瓦片地图重复平铺
- **修复**：重构多点巡航导航页（`waypoint_nav_page`），改用统一的 `TileMap` 组件，修复地图显示灰白色块及机器人位置坐标错误问题
- **新增**：多点巡航导航页支持**重定位**功能，与主页面保持一致（拖动机器人图标 → 确认 → 发布 `/initialpose`）
